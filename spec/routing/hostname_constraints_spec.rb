...
describe 'ルーティング' do
  ...
  example 'ホスト名が対象外ならばerrors/not_foundへ' do
    expect(get: 'http://foo.example.co.jp').to route_to(
      controller: 'errors',
      action: 'routing_error'
    )
  end

  example '存在しないパスならばerrors/not_foundへ' do
  ...
  example '利用者トップページ' do
    expect(get: 'http://example.com/customer').to route_to(
      host: 'example.com',
