...
RSpec.describe StaffMember, :type => :model do
  describe '#password=' do
    ...
    example 'パスワードにnilを与えると、hashed_passwordはnilになる' do
  ...
  describe 'バリデーション' do
    ...
    example 'family_nameに漢字・平仮名・片仮名・アルファベットは有効' do
      member = build(:staff_member, family_name: '漢あイAz')
      expect(member).to be_valid
    end
    
    example 'family_nameに記号は無効' do
    ...
    example '長音府を含むfamily_name_kanaは有効' do
      member = build(:staff_member, family_name_kana: 'メリー')
      expect(member).to be_valid
    end
